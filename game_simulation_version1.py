...
def game_simulation_v1(answer:str,max_iter=100,use_favor_init=True,p=0.04,q=-9):
    
    sum_tries = 0
    tries = np.zeros(8)
    for _ in np.arange(max_iter):
        num = game_simulation_v1_real(answer,use_favor_init=use_favor_init,p=p,q=q)
        sum_tries += num
        tries[num-1] += 1
...
def game_simulation_v1_real(answer:str,message=False,use_favor_init=True,p=0.04,q=-9):
    ...
    while(True):
        ...
        if(sim_word == answer or tries > 6):
            ...
            update_words(words,sim_word,answer,message,tries)
    return tries

def update_words(words,sim_word,answer,message=False,tries=None):
    ...
    if(message):
        print("tries:",tries,",left_words_num:",left_num)
